-- Hotfix: cast explícito para int em make_interval
...
begin
  ...
  with overdue as (
    select l.id,
           (row_number() over (order by l.next_action_at asc nulls last, l.id))::int as rn
    from public.cad_leads l
    where l.next_action_at < v_now
      and not exists (select 1 from public.cad_messages m where m.lead_id = l.id)
  ),
  upd as (
    update public.cad_leads l
    set next_action_at = v_now
                       + make_interval(days => ((o.rn - 1) / p_per_day)::int)
