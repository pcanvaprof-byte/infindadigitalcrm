...
-- Aplicar no SEU Supabase próprio (oxmhwwopxurwqcrwgsyf) via SQL Editor.
-- ============================================================
...
DROP POLICY IF EXISTS "Members can create api keys for their org" ON public.api_keys;
...
DROP POLICY IF EXISTS "Members can revoke their own keys; admins revoke any" ON public.api_keys;
...
CREATE INDEX IF NOT EXISTS idx_api_key_audit_org ON public.api_key_audit_log(organization_id, created_at DESC);
...
DROP POLICY IF EXISTS "Members can view org api audit" ON public.api_key_audit_log;
