-- ============================================================================
-- Hotfix: sobrescreve cad_admin_backfill_import_prospects sem prospects.organization_id
-- ============================================================================
-- Causa: a função aplicada no banco ainda tinha uma referência estática a
-- public.prospects.organization_id, coluna que não existe nessa base.
-- Estratégia: recriar a RPC com resolução de organização por caminhos válidos
-- e consultas dinâmicas apenas para tabelas opcionais.
...
declare
  v_org uuid;
begin
  -- 1) Reaproveita organização já presente em cad_leads do mesmo usuário.
  ...
  -- 2) Usa organização ativa, se a tabela existir.
  ...
  -- 3) Usa vínculo de membro, se a tabela existir.
  ...
  -- 4) Fallback seguro: primeira organização existente.
  select id into v_org
    from public.organizations
   order by created_at asc
   limit 1;

  -- 5) Se não existir nenhuma, cria a organização padrão sem depender de slug.
