...
export const Route = createFileRoute("/operacoes/clients/$id")({
  ssr: false,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 overflow-x-auto border-b border-border/40">
        {TABS.map((t) => {
          const target = t.to ? `/operacoes/clients/${id}/${t.to}` : `/operacoes/clients/${id}`;
          const active = t.to
            ? pathname.endsWith(`/${t.to}`)
            : pathname === `/operacoes/clients/${id}` || pathname === `/operacoes/clients/${id}/`;
          return (
            <Link
              key={t.to || "resumo"}
              to={target}
