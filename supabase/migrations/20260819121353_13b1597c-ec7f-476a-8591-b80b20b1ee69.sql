...
declare
  ...
  canonical_id uuid;
begin
  -- Prioridade 1: CNPJ (raiz de 8 dígitos)
  for r in (
    select public._prospect_norm_cnpj(cnpj) as cn, organization_id, count(*)
    ...
    from public.prospects 
    where public._prospect_norm_cnpj(cnpj) = r.cn 
  ...
  -- Prioridade 2: Nome + Cidade
  for r in (
    select public._prospect_norm_name(company) as nm, public._prospect_norm_city(city) as ct, organization_id, count(*)
    from public.prospects
    ...
    select id into canonical_id 
    from public.prospects 
    where public._prospect_norm_name(company) = r.nm 
      and public._prospect_norm_city(city) = r.ct
      and organization_id = r.organization_id 
      and merged_into is null
    order by created_at asc limit 1;

    update public.prospect_touchpoints set prospect_id = canonical_id 
    where prospect_id in (select id from public.prospects where public._prospect_norm_name(company) = r.nm and public._prospect_norm_city(city) = r.ct and organization_id = r.organization_id and id <> canonical_id);

    update public.cad_leads set prospect_id = canonical_id 
    where prospect_id in (select id from public.prospects where public._prospect_norm_name(company) = r.nm and public._prospect_norm_city(city) = r.ct and organization_id = r.organization_id and id <> canonical_id);

    update public.prospects set merged_into = canonical_id 
    where public._prospect_norm_name(company) = r.nm and public._prospect_norm_city(city) = r.ct and organization_id = r.organization_id and id <> canonical_id and merged_into is null;
  end loop;
end $$;
...
on public.prospects (organization_id, public._prospect_norm_cnpj(cnpj)) 
where merged_into is null and length(regexp_replace(coalesce(cnpj, ''), '\D', '', 'g')) >= 8;
