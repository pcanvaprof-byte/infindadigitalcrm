...
-- Helper: só Owner/Admin podem ver dados operacionais de todos na organização.
...
-- Helper usado por cad_messages/cad_notifications: Admin vê org; Member vê só lead próprio.
...
-- Cadência: remove políticas antigas e recria com escopo explícito por papel.
...
CREATE POLICY cad_leads_insert
  ON public.cad_leads
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY cad_messages_select
  ON public.cad_messages
  FOR SELECT
  TO authenticated
  USING (
    organization_id = public.current_org_id()
...
CREATE POLICY cad_messages_insert
  ON public.cad_messages
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY cad_messages_update
  ON public.cad_messages
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY cad_messages_delete
  ON public.cad_messages
  FOR DELETE
  TO authenticated
  USING (
...
CREATE POLICY cad_notif_select
  ON public.cad_notifications
  FOR SELECT
  TO authenticated
  USING (
    organization_id = public.current_org_id()
...
-- Prospecção: substitui política org-wide restritiva por isolamento real por papel.
...
CREATE POLICY touchpoints_scope_by_role
  ON public.prospect_touchpoints
  FOR ALL
  TO authenticated
  USING (
    organization_id = public.current_org_id()
    AND (public._is_org_admin() OR user_id = auth.uid())
  )
  WITH CHECK (
    organization_id = public.current_org_id()
...
CREATE POLICY interactions_scope_by_role
...
-- Registra o autor real do disparo e bloqueia Member de registrar em lead alheio.
...
BEGIN
  ...
  INSERT INTO public.cad_messages (
    lead_id, organization_id, author_id, tipo, direction, stage_at_send, mensagem, status
  )
  VALUES (
    p_lead, v_lead.organization_id, auth.uid(), p_tipo, 'out', v_lead.stage, p_mensagem, 'enviada'
  )
  ...
  IF p_advance AND v_lead.stage::text LIKE 'followup_%' THEN
    ...
    UPDATE public.cad_leads
       SET last_contact_at = now(), stage = v_next_stage, next_action_at = v_next_at
     WHERE id = p_lead;
  ELSE
    UPDATE public.cad_leads
...
-- Dashboard de Cadência: Member só soma seus próprios leads/mensagens.
...
BEGIN
  ...
  FROM (
    SELECT l.stage::text AS stage, count(*) c
    FROM public.cad_leads l
    WHERE l.organization_id = v_org
      AND (v_admin OR l.owner_id = v_uid)
    GROUP BY l.stage
