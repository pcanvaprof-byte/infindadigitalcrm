...
CREATE INDEX IF NOT EXISTS idx_user_access_events_user ON public.user_access_events(user_id, created_at DESC);
...
CREATE POLICY user_access_events_select_self_or_admin ON public.user_access_events
  FOR SELECT TO authenticated
  USING (user_id = auth.uid() OR (organization_id IS NOT NULL AND public.is_org_admin(organization_id)));
