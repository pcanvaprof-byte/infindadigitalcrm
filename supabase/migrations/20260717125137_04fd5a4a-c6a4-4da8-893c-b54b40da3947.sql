...
begin
  ...
  with
  p as (
    select * from prospects where organization_id = v_org
  ),
  uls as (
  ...
  select jsonb_build_object(
    ...
    'gargalos', jsonb_build_object(
      ...
      -- Métrica gerencial: só aparece para admin/owner. Membros comuns veem 0.
      'sem_responsavel', case when v_is_admin
        then (select count(*) from p where coalesce(nullif(owner_name,''), null) is null)
        else 0 end,
